...
function retryCountFromEnv() {
  return Number.parseInt(process.env.LEAK_HUNTER_RELEASE_ASSET_RETRIES ?? '1', 10);
}

function retryDelayMsFromEnv() {
  return Number.parseInt(process.env.LEAK_HUNTER_RELEASE_ASSET_RETRY_DELAY_MS ?? '1000', 10);
}
...
async function verifyReleaseAssets({
  ...
  throw new Error(
    [
      ...
      'Create and host the GitHub release with cargo-dist before publishing npm.',
    ].join('\n'),
  );
}
